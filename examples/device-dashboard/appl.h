...
struct Dehumi_t {
  ...
  double Temp;
  double Humi;
  double HumiStart;  // 湿度启动值
  double HumiStop;   // 湿度停止值
  double HeatStart;  // 加热启动值
  int HeatState;     // 加热状态
  char szHeatState[128];
  int WorkState;  // 除湿机工作状态
  char szWorkState[256];
  int ManOrAuto;  // 除湿手自动切换 00 自动，01 手动
  char szManOrAuto[128];
  int ManHumi;
  int ManHeat;
