...
static void handle_settings_set(struct mg_connection *c, struct mg_str body) {
  ...
  struct chanmqtt_t *mqtt = &APPL.chanmqtt[4];
  int cmd;
  int param;
  bool ok = true;
  int rc;
...
static void handle_env_set(struct mg_connection *c, struct mg_str body) {
  long cmd;
  long param;
  bool ok = true;
  ...
  if( cmd == -1 || param == -1){
    ...
    switch (cmd){
    case ENV_CMD_SET_DH_HUMISTART:
      ch->Cmd = CMD_485_DH_SET_HUMISTART;
      ch->CmdParam = param;
      break;

    case ENV_CMD_SET_DH_HUMISTOP:
      ch->Cmd = CMD_485_DH_SET_HUMISTOP;
      ch->CmdParam = param;
      break; 
...
static void handle_comm_get(struct mg_connection *c) {
  ...
  struct chanmqtt_t *c7 = &APPL.chanmqtt[4];
  mg_http_reply(c, 200, s_json_header, "{\
...
%m:{%m:%m,%m:%m,%m:%f,%m:%f,%m:%f,%m:%f,%m:%f,%m:%m,%m:%m,%m:%m,%m:%d,%m:%d},\
%m:{%m:%m,%m:%m,%m:%d,%m:%m}}\n",
...
static void fn(struct mg_connection *c, int ev, void *ev_data) {
  if (ev == MG_EV_ACCEPT) {
    ...
    if ( 0 ) { // mg_http_match_uri(hm, "/api/#") && u == NULL
      ...
      handle_env_get(c);
    }else if (mg_http_match_uri(hm, "/api/env/set")) {
      handle_env_set(c, hm->body);
